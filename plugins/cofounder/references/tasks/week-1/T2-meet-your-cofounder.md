...
- At the start of every session: run the **cf-continue** skill. It will tell you both
  where things stand.
...
At the start of every session, run the cf-continue skill.
```
...
> Want me to check in once a day? It's one short message that just runs **cf-continue**. If
